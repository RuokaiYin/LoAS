...
def _pe_operate(pe_x, pe_w, stats_dict):
    ...
    stats_dict['dram_traffic_o'] += w_nzcol*2 #! 2 times, 1 for reading to merge, 1 for writing
    ...
    for k,v in layer_stats.items():
        ...
        if 'traffic' in k:
            ...
            if 'dram_traffic' in k:
                if k[-1] == 'o': #! We need to scale the partial sum traffic according to the timesteps, due to partial row SRAM, detail comments in _pe_operate 
                    layer_stats[k] = layer_stats[k]*(timestep-3)/timestep
                    layer_stats['total_dram'] += v*(bit_dict[k[-1]])/(1024*8)*(timestep-3)/timestep
                else:
...
def gamma_sim_layer(inp_mat, w_mat, tiling_dataflow, tile_dict):
    ...
    layer_stats[f'dram_traffic_x_a'] += torch.count_nonzero(inp_mat)
...
if __name__ == '__main__':
    ...
    # torch.save(results, f'./gamma_{args.arch}_dict.pth')
