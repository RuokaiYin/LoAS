...
def plot_layerwise(arch, mode, pmode):
    #########################################
    # Data 
    #########################################
    # arch = 'alexnet'
    ...
    gamma_sram_o = 0
    gamma_sram_others = 0
    gamma_cache_miss_x = 0
    gamma_cache_miss_w = 0
    for k,v in dic.items():
        if k == lay:
            gamma_cycles += v['tot_cycs']
            gamma_dram += v['total_dram']
            gospa_sram += v['total_sram']
            gamma_dram_w += v['dram_traffic_w']
            gamma_dram_x += v['dram_traffic_x']
            gamma_dram_o += v['dram_traffic_o']
            gamma_dram_others += v['dram_traffic_x_a']
            gamma_dram_others += v['dram_traffic_w_b']
                    
            gamma_sram_w += v['sram_traffic_w']
            gamma_sram_x += v['sram_traffic_x']
            gamma_sram_o += v['sram_traffic_o']
            gamma_sram_others += v['sram_traffic_x_a']
            gamma_sram_others += v['sram_traffic_w_b']

            gamma_cache_miss_x += v['miss_x']/(v['hit_x']+v['miss_x'])*100
            gamma_cache_miss_w += v['miss_w']/(v['hit_w']+v['miss_w'])*100
    ...
    # Plot 
    #########################################
    labels = ['SparTen-S', 'GoSpa-S', 'Gamma-S', 'LoAS']
    if mode == 'dram':
        values_w = [sparten_dram_w/loasft_dram, gospa_dram_w/loasft_dram, gamma_dram_w/loasft_dram, loasft_dram_w/loasft_dram]
        values_x = [sparten_dram_x/loasft_dram, gospa_dram_x/loasft_dram, gamma_dram_x/loasft_dram, loasft_dram_x/loasft_dram]
        values_o = [sparten_dram_o/loasft_dram, gospa_dram_o/loasft_dram, gamma_dram_o/loasft_dram, loasft_dram_o/loasft_dram]
        values_ot = [sparten_dram_others/loasft_dram, gospa_dram_others/loasft_dram, gamma_dram_others/loasft_dram, loasft_dram_others/loasft_dram]
        # Create bar chart
        fig, ax = plt.subplots(1,1,figsize=(1.2,1.8),dpi=150)
        bars = ax.bar(labels, values_w, color='#F4CE14',linewidth=0.9, zorder=4, edgecolor='black',alpha=0.85)
        bars = ax.bar(labels, values_x, bottom =values_w, color='#A3B763',linewidth=0.9, zorder=4, edgecolor='black',alpha=0.85)
        bars = ax.bar(labels, values_o, bottom =np.array(values_w)+np.array(values_x), color='#557C55',linewidth=0.9, zorder=4, edgecolor='black',alpha=0.85)
        bars = ax.bar(labels, values_ot, bottom =np.array(values_w)+np.array(values_x)+np.array(values_o), color='#FD841F',linewidth=0.9, zorder=4, edgecolor='black',alpha=0.85)
    elif mode == 'sram':
        values_w = [sparten_sram_w/loasft_sram, gospa_sram_w/loasft_sram, gamma_sram_w/loasft_sram, loasft_sram/loasft_sram]
        values_x = [sparten_sram_x/loasft_sram, gospa_sram_x/loasft_sram, gamma_sram_x/loasft_sram, loasft_sram/loasft_sram]
        values_o = [sparten_sram_o/loasft_sram, gospa_sram_o/loasft_sram, gamma_sram_o/loasft_sram, loasft_sram/loasft_sram]
        values_ot = [sparten_sram_others/loasft_sram, gospa_sram_others/loasft_sram, gamma_sram_others/loasft_sram, loasft_sram_others/loasft_sram]
        # Create bar chart
        fig, ax = plt.subplots(1,1,figsize=(1.2,1.8),dpi=150)
        bars = ax.bar(labels, values_w, color='#F4CE14',linewidth=0.9, zorder=4, edgecolor='black',alpha=0.85)
        bars = ax.bar(labels, values_x, bottom =values_w, color='#A3B763',linewidth=0.9, zorder=4, edgecolor='black',alpha=0.85)
        bars = ax.bar(labels, values_ot, bottom =np.array(values_w)+np.array(values_x), color='#FD841F',linewidth=0.9, zorder=4, edgecolor='black',alpha=0.85)
    elif mode == 'miss':
        miss_x = [sparten_cache_miss_x/loasft_cache_miss_x, gospa_cache_miss_x/loasft_cache_miss_x, gamma_cache_miss_x/loasft_cache_miss_x, loasft_cache_miss_x/loasft_cache_miss_x]
        # miss_w = [sparten_cache_miss_w,gospa_cache_miss_w,loasft_cache_miss_w]
        width = 0.2
        # x= np.array([1,2,3])
        fig, ax = plt.subplots(1,1,figsize=(1.2,1.8),dpi=150)
        bars = ax.bar(labels, miss_x, color='#B99470',linewidth=0.9, zorder=4, edgecolor='black',alpha=0.85, width=0.9)
        ax.set_ylim(0,2)
        ax.set_yticklabels([])
    ...
    if arch == 'alexnet':
        ax.set_title('Alexnet-L4')
    elif arch == 'vgg16':
        ax.set_title('VGG16-L8')
    elif arch == 'resnet19':
    ...
    # ax.set_yticklabels([])
    # ax.set_ylim(1,20)
    # ax.set_xticklabels(['SparTen-S', 'GoSpa-S', 'Gamma-S', 'LoAS'])
    ...
    if pmode == 'show':
        ...
        plt.savefig(f'../figures/revision/{arch}_layerwise_{mode}.pdf', bbox_inches='tight',pad_inches=0.1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, default='speedup', help='[dram, sram, miss]')
    parser.add_argument('--pmode', type=str, default='show', help='[show, save]')
    args = parser.parse_args()
    plot_layerwise(args.arch, args.mode, args.pmode)
