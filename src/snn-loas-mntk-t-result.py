...
def print_layer_stats(layer_stats, t=4):
...
def loas_sim_layer(inp_mat, w_mat, tiling_dataflow, tile_dict, t=4):
    ...
    print_layer_stats(layer_stats,t)

    return layer_stats

def test_real_data(path, t):
    ...
    for k,v in matrices_dict.items():
        ...
        result_dict[k] = loas_sim_layer(inp_mat, w_mat, tiling_dataflow, tile_dict, t)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--T', type=int, default=8, help='[6, 8]')
    args = parser.parse_args()

    results = test_real_data(f'../matrices_dict/{args.arch}_{args.spar_lvl}_T{args.T}_final_matrices_dict_loas_{args.loas}.pth', args.T)
    torch.save(results, f'./loas-mntk_{args.loas}_{args.arch}_{args.spar_lvl}_T{args.T}_dict.pth')
