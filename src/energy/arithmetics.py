...
#! This returns depth 16 FIFO with input bitwidth, remeber to scale the FIFO with depth linearly.
class FIFO(Primitive_Computation_Unit):
    def __init__(self, name, bit_width):
        super().__init__(name, bit_width)
        self.name = name
        self.bit_width = bit_width
        self.check_synthesis_file('FIFO',synthesis_file_location)

class Comparator(Primitive_Computation_Unit):
    def __init__(self, name, bit_width):
        super().__init__(name, bit_width)
        self.name = name
        self.bit_width = bit_width
        self.check_synthesis_file('COMP',synthesis_file_location)

class ScratchPad(Primitive_Computation_Unit):
    def __init__(self, name, bit_width):
        super().__init__(name, bit_width)
        self.name = name
        self.bit_width = bit_width
        self.check_synthesis_file('SPAD',synthesis_file_location)
