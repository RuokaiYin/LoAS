...
def _tile_to_pe_custom(inp_tile, w_tile):
    ...
    #! In SparTen, the inter-PE dimensions is w.size(1)
    #! In SparTen, the inter-PE dim for x is 0, meaning all PEs shared same inputs.
    activated_pe = 0
    for i in range(n_pe):
...
def _pe_operate(pe_x, pe_w, stats_dict):
    ...
    stats_dict['sram_traffic_w'] += effect
...
def _check_dram2sram_traffic(tensor, sram_buffer, sram_size_left, stats_dict, var=None):
    ...
    if found:
        ...
        if var == 'w':
            stats_dict[f'dram_traffic_wb'] += n_tot
            stats_dict[f'sram_traffic_wb'] += n_tot
            stats_dict[f'dram_traffic_{var}'] += n
            stats_dict[f'sram_traffic_{var}'] += n
        elif var == 'x':
            stats_dict[f'dram_traffic_{var}'] += n_tot
            stats_dict[f'sram_traffic_{var}'] += n_tot
        else:
...
def sparten_sim_layer(inp_mat, w_mat, tiling_dataflow, tile_dict):
    ...
    sram_size_w = 128*1024
    sram_size_x = 128*32*4
    ...
    layer_stats['dram_traffic_o'] += inp_mat.numel()
    for i in tqdm(range(len(x_tiles))):
        ...
        if not torch.equal(w_tiles[i], w_tiles[i-1]):         
...
def test_real_data(path):
    ...
    for k,v in matrices_dict.items():
        ...
        result_dict[k] = sparten_sim_layer(inp_mat, w_mat, tiling_dataflow, tile_dict)
...
if __name__ == '__main__':
    ...
    torch.save(results, f'./sparten_{args.arch}_dict.pth')
